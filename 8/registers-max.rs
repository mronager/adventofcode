...
impl Register {
    fn instruct(&mut self, instruction: &str) {
        ...
        if multiply {
            let v = val_a + (val * multiplier);
            self.registers.insert(register_a.to_owned(), v);
...
fn main() {
    let stdin = io::stdin();
    let mut registers = Register { registers: HashMap::new(), max: 0 };
...
#[test]
fn test_1() {
    let mut register = Register { registers: HashMap::new(), max: 0 };
...
fn test_3() {
    ...
    register.instruct("c dec -10 if a >= 1");
    register.instruct("c inc -20 if c == 10");
    assert_eq!(10isize, register.get_largest());
}
